...
\title{
    Agent-Based Modelling in Cellular Biology\\We are not there yet
...
\renewcommand{\shorttitle}{Agent-Based Modelling in Cellular Biology - We are not there yet}
...
\hypersetup{
pdftitle={Agent-Based Modelling in Cellular Biology - We are not there yet},
...
Every individual component of these tools can be formulated using mathematical methods despite that
many results can not be simply calculated by a purely mathematical approach.
...
One thing that unifies these \acp{abm} is that each of them assumes a particular spatial
...
These are limitations on the ability to use model reduction techniques within the \ac{abm} and make
it difficult to follow best principles such as Occam's Razor~\cite{Sober2015} for model development.
...
When deciding on this approach it is still possible to utilize existing tools as long as they are
...
However, due to their general nature, they may not contain the desired functionalities which then
need to be built by the researchers leading to additional human developmental effort in the
