...
\section{Constructing Cellular \aclp{abm}}
...
\ac{abm} and make it difficult to follow best principles such as Ockam's Razor~\cite{Sober2015} for
...
In order for this approach to be valuable, researchers need to be able to construct external building blocks without having to rely on existing functionalities.
Furthermore we believe that widespread adoption can only be achieved if there exist a large amount of predefined building blocks which can be reused by researchers.
This procedure is well known to most software developers who have authored one or more libraries
where it is desirable to provide reusable parts that can be combined in multiple ways.\\
...
Although it exhibits groundbreaking performance it is yet to be applied across a diverse range of topics and only supports a limited set of building blocks at the time of writing.
